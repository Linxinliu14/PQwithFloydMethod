...
public class PriorityQueue<K> implements Iterable<K> {
    ...
    private void snake(@SuppressWarnings("SameParameterValue") int k) {
        ...
        while (firstChild(i) <= last) {
            int j = firstChild(i);
            if (comparator.compare(binHeap[j], binHeap[j + 1]) > 0) {
                swap(i, j);
            } else {
                swap(i, j + 1);
            }
    ...
    public static void main(String[] args) throws PQException {
        ...
        PriorityQueue<Integer> PQ_int_floyd = new PriorityQueue<Integer>(max, s2, 5, Comparator.comparing(Integer::intValue), floyd);
        PriorityQueue<Integer> PQ_int_nofloyd = new PriorityQueue<Integer>(max, s2, 5, Comparator.comparing(Integer::intValue), false);
